...
fn ps_opaque(arg: VertexOut) -> @location(0) vec4<f32> {
  return arg.color;
    ...
    fxe::pipeline_key key{};
    key.vs_entry = "vs_transform";
    key.fs_entry = "ps_opaque";
    key.color_format = wgpu::TextureFormat::BGRA8Unorm;
    key.depth_format = wgpu::TextureFormat::Undefined;
    key.blend = fxe::blend_mode::alpha;
    key.topology = wgpu::PrimitiveTopology::TriangleList;
    key.sample_count = 1;

    wgpu::RenderPipeline first = cache.acquire(key, device, layout, shader);
    wgpu::RenderPipeline second = cache.acquire(key, device, layout, shader);
    if (!first || !second)
