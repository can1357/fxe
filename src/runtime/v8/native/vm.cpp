...
namespace fxe::runtime {
  namespace {
    ...
    std::string utf8(Isolate* iso, Local<Value> value) {
      String::Utf8Value text(iso, value);
    ...
    void throw_type(Isolate* iso, std::string_view message) {
    ...
    Maybe<bool> validate_runtime_opts(Isolate* iso, Local<Context> ctx, Local<Value> opts_value) {
      ...
      if (!opts_value->IsObject()) {
        throw_type(iso, "vm options must be an object");
        return Nothing<bool>();
      }
      auto opts = opts_value.As<Object>();
      Local<Value> microtask_mode;
      if (!opts->Get(ctx, "microtaskMode"_v8(iso)).ToLocal(&microtask_mode))
        return Nothing<bool>();
      if (!microtask_mode->IsUndefined() && !microtask_mode->IsNull()) {
        const std::string mode = utf8(iso, microtask_mode);
    ...
    Maybe<bool> parse_script_options(Isolate* iso, Local<Context> ctx, Local<Value> opts_value,
      ...
      Local<Value> filename_value;
      if (!opts->Get(ctx, "filename"_v8(iso)).ToLocal(&filename_value))
        return Nothing<bool>();
      if (!filename_value->IsUndefined() && !filename_value->IsNull())
        out.filename = utf8(iso, filename_value);

      Local<Value> line_value;
      if (!opts->Get(ctx, "lineOffset"_v8(iso)).ToLocal(&line_value))
        return Nothing<bool>();
      if (!line_value->IsUndefined() && !line_value->IsNull())
        out.line_offset = line_value->Int32Value(ctx).FromMaybe(0);

      Local<Value> column_value;
      if (!opts->Get(ctx, "columnOffset"_v8(iso)).ToLocal(&column_value))
        return Nothing<bool>();
    ...
    ScriptOrigin make_origin(Isolate* iso, const script_options& opts) {
      return ScriptOrigin(str(iso, opts.filename), opts.line_offset, opts.column_offset,
    ...
    Maybe<bool> property_is_enumerable(Isolate* iso, Local<Context> ctx, Local<Object> obj,
      ...
      if (!desc_value.As<Object>()->Get(ctx, "enumerable"_v8(iso)).ToLocal(&enumerable))
        return Nothing<bool>();
    ...
    Maybe<bool> apply_props(Isolate* iso, Local<Context> target_ctx, Local<Object> to,
      ...
      for (const auto& prop : props) {
        if (skip_native_binding && prop.key->IsString() && utf8(iso, prop.key) == "__fxe_native")
          continue;
    ...
    MaybeLocal<Context> context_from_value(Isolate* iso, Local<Value> value) {
      if (!is_tagged_object(value, kContextTag)) {
        throw_type(iso, "vm context must be created by vm.createContext()");
        return {};
      }
    ...
    void script_run_in_context(const FunctionCallbackInfo<Value>& info) {
      ...
      if (info.Length() < 1 || !info[0]->IsObject()) {
        throw_type(iso, "Script.runInContext(contextifiedObject[, options]) requires a context");
        return;
      }
    ...
    void script_run_in_new_context(const FunctionCallbackInfo<Value>& info) {
      ...
      if (info.Length() > 0 && !info[0]->IsUndefined() && !info[0]->IsNull()) {
        if (!info[0]->IsObject()) {
          throw_type(iso, "Script.runInNewContext([sandbox[, options]]) sandbox must be an object");
          return;
        }
    ...
    MaybeLocal<Object> make_compiled_script_object(Isolate* iso, Local<Context> ctx,
      ...
      holder->SetInternalField(0, fxe::js::make_external(iso, state));
      holder->SetInternalField(1, Uint32::New(iso, kScriptTag));
      state->bind(iso, holder);
      (void)holder->Set(ctx, "runInThisContext"_v8(iso),
                        Function::New(ctx, script_run_in_this_context).ToLocalChecked());
      (void)holder->Set(ctx, "runInContext"_v8(iso),
                        Function::New(ctx, script_run_in_context).ToLocalChecked());
      (void)holder->Set(ctx, "runInNewContext"_v8(iso),
                        Function::New(ctx, script_run_in_new_context).ToLocalChecked());
      return holder;
    }
    ...
    MaybeLocal<Object> make_context_object(Isolate* iso, Local<Context> ctx, Local<Object> seed) {
      ...
      global->SetInternalField(0, fxe::js::make_external(iso, state));
      global->SetInternalField(1, Uint32::New(iso, kContextTag));
      return global;
    }
    ...
    void native_run_in_this_context(const FunctionCallbackInfo<Value>& info) {
      ...
      if (info.Length() < 1 || !info[0]->IsString()) {
        throw_type(iso, "__fxe_native.vm.runInThisContext(code[, options]) requires code");
        return;
      }
      script_options opts;
    ...
    void native_run_in_new_context(const FunctionCallbackInfo<Value>& info) {
      ...
      if (info.Length() < 1 || !info[0]->IsString()) {
        throw_type(iso,
                   "__fxe_native.vm.runInNewContext(code[, sandbox[, options]]) requires code");
        return;
      }
      Local<Object> sandbox = Object::New(iso);
      if (info.Length() > 1 && !info[1]->IsUndefined() && !info[1]->IsNull()) {
        if (!info[1]->IsObject()) {
          throw_type(iso, "vm sandbox must be an object");
          return;
        }
    ...
    void native_compile(const FunctionCallbackInfo<Value>& info) {
      ...
      auto ctx = iso->GetCurrentContext();
      if (info.Length() < 1 || !info[0]->IsString()) {
        throw_type(iso, "__fxe_native.vm.compile(code[, options]) requires code");
        return;
      }
      script_options opts;
      if (!parse_script_options(iso, ctx, info.Length() > 1 ? info[1] : Local<Value>(), opts)
    ...
    void native_create_context(const FunctionCallbackInfo<Value>& info) {
      ...
      if (info.Length() > 0 && !info[0]->IsUndefined() && !info[0]->IsNull()) {
        if (!info[0]->IsObject()) {
          throw_type(iso, "__fxe_native.vm.createContext([sandbox]) sandbox must be an object");
          return;
        }
    ...
    void native_compile_function(const FunctionCallbackInfo<Value>& info) {
      ...
      for (uint32_t i = 0; i < params->Length(); ++i) {
        ...
        if (!params->Get(ctx, i).ToLocal(&param))
          return;
        wrapped += utf8(iso, param);
      }
      wrapped += "){ ";
      wrapped += utf8(iso, info[0]);
      wrapped += " })";
      Local<Value> result;
      if (!run_string_in_context(ctx, str(iso, wrapped), opts).ToLocal(&result))
        return;
      info.GetReturnValue().Set(result);
    }
    ...
    Local<Object> make_vm_namespace(Isolate* iso, Local<Context> ctx) {
      ...
      (void)ns->Set(ctx, "runInThisContext"_v8(iso),
                    Function::New(ctx, native_run_in_this_context).ToLocalChecked());
      (void)ns->Set(ctx, "runInContext"_v8(iso),
                    Function::New(ctx, native_run_in_context).ToLocalChecked());
      (void)ns->Set(ctx, "runInNewContext"_v8(iso),
                    Function::New(ctx, native_run_in_new_context).ToLocalChecked());
      (void)ns->Set(ctx, "compile"_v8(iso), Function::New(ctx, native_compile).ToLocalChecked());
      (void)ns->Set(ctx, "createContext"_v8(iso),
                    Function::New(ctx, native_create_context).ToLocalChecked());
      (void)ns->Set(ctx, "isContext"_v8(iso),
                    Function::New(ctx, native_is_context).ToLocalChecked());
      (void)ns->Set(ctx, "compileFunction"_v8(iso),
                    Function::New(ctx, native_compile_function).ToLocalChecked());
      return ns;
    }
  } // namespace
  ...
  void install_native_vm(Isolate* iso, Local<Context> ctx) {
    ...
    (void)native->Set(ctx, "vm"_v8(iso), make_vm_namespace(iso, ctx));
  }
} // namespace fxe::runtime
